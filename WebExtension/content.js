...
// import { SDModelParser, SDModelInfo, SDModelType } from '../src/models';
//
// const metadata = {
...
// };
//
// const modelInfo = SDModelParser.parseFromMetadata(metadata);

(async () => {
    // The ExifReader library uses a Universal Module Definition (UMD) pattern
    await import(chrome.runtime.getURL('lib/exifreader.js'));
    const ExifReader = globalThis.ExifReader;

    const Translate = await import(chrome.runtime.getURL('./utils/translate.js'));

    const { getBgImgs, getSVGEmbeddedImages, updateBgAlternatives } = await import(
        chrome.runtime.getURL('./deepsearch/bg_images.js')
    );
    ...
    async function getDescription(name, tag) {
        ...
        if (Array.isArray(tag)) {
            ...
            console.log(
                'falling back to  tag.description: ' +
                    tag.description +
                    ' is not a DataView it has typeof ' +
                    typeof tag.value
            );
    ...
    async function loadparse(imgrequest) {
        ...
        if (imgrequest.jpegURL) {
            infosArr.push(
                'Image shown on webpage is in ' +
                    imgrequest.imageType.replace('image/', '') +
                    ' format. An alternative image was detected (assumed similar) to look for meta-data in...' +
                    (imgrequest.proxyType ? imgrequest.proxyType.replace('image/', '') : '')
            );
    ...
    async function loadparseshow(imgrequest) {
        // handleChosenOne
        ...
        function handleResult(result) {
            ...
            if (
                imgrequest.naturalWidth &&
                imgrequest.goDeepSearch &&
                imgrequest.supportsDeepSearchModifier &&
                !imgrequest.deepSearchBigger &&
                imgrequest.naturalWidth * imgrequest.naturalHeight <=
                    imgrequest.deepSearchBiggerLimit
            ) {
                infosArr.push(
                    'Not the expected image? You can force xIFr to look for a larger image than this, by holding down Shift key when selecting xIFr in the context menu!'
                );
            }
        ...
        if (
            imgrequest.fetchMode === 'devFrontendFetch' ||
            (imgrequest.fetchMode === 'devAutoFetch' &&
                (pageOrigin === imgOrigin ||
                    true ||
                    imgOrigin.startsWith('data:') ||
                    imgOrigin.startsWith('blob:') ||
                    imgOrigin.startsWith('file:')))
        ) {
            // Do frontend fetch...
            ...
            if (context.isFirefox()) {
                // Fastest Structured clone algorithm. Supported by Firefox
                context.debug(
                    'fetchdata: Receiving from backend as Uint8Array-arrayBuffer by the Structured clone algorithm (The fastest way, and supported by Firefox)'
                );
                browser.runtime
                    .sendMessage({
                        message: 'fetchdata',
                    ...
                    .catch((error) =>
                        console.error(
                            'xIFr: fetchdata backend fetch - There has been a problem with your fetch operation: ',
                            error.message,
                            error
                        )
                    );
            } else {
                // Slower JSON serialization algorithm. Supported by Chromium browsers (Also used to work with Firefox, but not anymore with MV3)
                context.debug(
                    'fetchdataBase64: Receiving from backend as base64 by the JSON serialization algorithm (The widely supported way, and supported by both Chromium and Firefox)'
                );
                browser.runtime
                    .sendMessage({
                    ...
                    .catch((error) =>
                        console.error(
                            'xIFr: fetchdataBase64 backend fetch - There has been a problem with your fetch operation: ',
                            error.message,
                            error
                        )
                    );
    ...
    if (typeof globalThis.contentListenerAdded === 'undefined') {
        browser.runtime.onMessage.addListener((request) => {
            if (request.message === 'showInfos') {
                ...
                if (image) {
                    ...
                    extraImages.then((xtrSizes) => {
                        ...
                        var image2 = deeperSearch(
                            request,
                            elem,
                            xtrSizes.sort((a, b) => (b.weight || 1) - (a.weight || 1))
                        );
                    });
                }
                ...
                if (request.goDeepSearch) {
                    ...
                    if (elem) {
                        ...
                        logDSEARCH &&
                            console.log(
                                `xIFr: The righclicked element is a <${request.nodeName} /> (${elem}) found on ${elem.ownerDocument.documentURI} (${elem.ownerDocument}).`
                            );
                        logDSEARCH &&
                            console.log(
                                `xIFr: The DocumentElement is a : <${elem.ownerDocument.documentElement?.nodeName?.toLowerCase()} /> (${
                        ...
                        if (elem.shadowRoot) {
                            logDSEARCH &&
                                console.warn(
                                    'xIFr: The rightclicked element hosts a shadowDOM which may be invisible for current version of the Deep Search algorithm!'
                                );
                        }
                        const extraLoads = [];
                        let rootNode = elem.getRootNode({ composed: false });
                        ...
                        logDSEARCH &&
                            console.log(
                                `xIFr: A contextscript running on ${document.documentURI} (${document}) did not get a target element id!`
                            );
                        context.debug(
                            `xIFr: A contextscript running on ${document.documentURI} (${document}) did not get a target element id`
                        );
                    ...
                    if (img) {
                        ...
                        logDSEARCH &&
                            console.warn(
                                'xIFr: Simple search did not find a match in document.images'
                            );
                    }
                    loadparseshow(image);
                } else {
                ...
                console.log(
                    'xIFr: Received message displayInPage with data ' + JSON.stringify(request.data)
                );
                ...
                if (dialog) {
                    ...
                    dialog.setAttribute(
                        'style',
                        'box-sizing:border-box; max-width:90svw; max-height:90svh; padding:0; margin:auto; border:none; box-shadow: rgba(50, 52, 55, 0.2) 0 6px 18px;overflow:auto;pointer-events:auto;user-select:auto;'
                    );
                    let img = document.createElement('img');
                    img.setAttribute('src', request.data.URL);
                    img.setAttribute(
                        'style',
                        'padding:0;margin:0;border:none;max-width:90svw;display:block;aspect-ratio:auto;pointer-events:auto;user-select:auto;'
                    );
            ...
            return Promise.resolve(
                `The contentscript says thanks for the '${request.message}' message! 😊`
            );
        });
