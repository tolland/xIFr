...
if (browser.menus?.getTargetElement) {
    // An easy way to use Firefox extended API while preserving Chrome (and older Firefox) compatibility.
...
browser.runtime.onInstalled.addListener(({ reason }) => {
    console.debug('handling install for reason ' + reason);
    if (reason === 'update') {
...
browser.commands.onCommand.addListener((command) => {
    console.log('got here' + command);
    browser.tabs.create({ url: 'https://developer.mozilla.org' });
});

browser.runtime.onConnect.addListener(({ port }) => {
    console.debug('handling connect with port ' + port);
});

browser.runtime.onInstalled.addListener(function handleInstalled({
    reason,
    temporary,
    previousVersion,
}) {
    console.error('in handleInstalled');
    context.getOptions().then(function (options) {
        createMenuItem(
            !options.devDisableDeepSearch &&
                browser.contextMenus.getTargetElement
        );
    });

    switch (reason) {
        case 'update': // "upboarding"
            console.debug('handling update of version ' + previousVersion);
            break;
        case 'install': // "onboarding"
            console.debug('handling install of version ' + previousVersion);
            break;
    }
});

browser.runtime.onStartup.addListener(() => {
    console.error('in onStartup');
    context.getOptions().then(function (options) {
...
context.getOptions().then(function (options) {
    createMenuItem(
        !options.devDisableDeepSearch && browser.contextMenus.getTargetElement
    );
});
...
function onBrowerActionClicked(details) {
    console.log("handling browser action 'onClicked' event");
    browser.notifications.create('onInstalled', {
        title: 'This is a title of a message',
        message: `onInstalled has been called, background page loaded at ${loadTime.getHours()}:${loadTime.getMinutes()}`,
        type: 'basic',
    });
}

browser.action.onClicked.addListener(onBrowerActionClicked);

browser.contextMenus.onClicked.addListener((info, tab) => {
    if (info.menuItemId === 'showinfos') {
        context.debug('Context menu clicked. mediaType=' + info.mediaType);

        const scripts = [
            'context.js', // Some state and options handling, utility functions
            'parsers/decoders.js', // Translation handling
            'utils/stringBundle.js', // Translation handling
            'utils/fxifUtils.js', // Some utility functions
            'parsers/binExif.js', // Interpreter for binary EXIF data
            'parsers/binIptc.js', // Interpreter for binary IPTC-NAA data
            'parsers/xmp.js', // Interpreter for XML XMP data
            'parsers/parseJpeg.js', // "Master parser" for header-data
            'content.js', // "Conductor" frontend script (frontend-fetch of image, communication with backend, "deep search" functionality)
            //"ExifReader/dist/exif-reader.js" // "Master" frontend script (popup creation, data handling, etc.)
        ];
        if (browser.scripting?.executeScript) {
            ...
            Promise.all([context.getOptions(), scriptsInjecting])
                .then((values) => {
                    context.debug(
                        'All scripts started from background is ready...'
                    );
                    const options = values[0];
                    if (!values[1].length || !values[1][0]) {
                        console.error(
                            'xIFr: There was an error loading contentscripts: ' +
                ...
                .catch((err) => {
                    console.error(
                        `xIFr: Failed getting data or injecting scripts: ${err}`
                    );
                });
        }
        browser.tabs
            ...
            .catch((e) => {
                console.error('xIFr: Sending showInfos message failed! \n' + e);
            });
    } else if (info.menuItemId === 'viewexif') {
        context.debug('Context menu clicked. mediaType=' + info.mediaType);
        // https://developer.mozilla.org/en-US/docs/Mozilla/Add-ons/WebExtensions/API/menus/OnClickData
        if (
            (info.mediaType && info.mediaType === 'image' && info.srcUrl) ||
            ...
            if (browser.scripting?.executeScript) {
                ...
                Promise.all([context.getOptions(), scriptsInjecting])
                    .then((values) => {
                        ...
                        browser.storage.session
                            ...
                            .then(() => {
                                browser.tabs
                                    .sendMessage(tab.id, {
                                        ...
                                        targetId: info.targetElementId,
                                        supportsDeepSearch:
                                            !!info.targetElementId, // "deep-search" supported in Firefox 63+
                                        goDeepSearch:
                                            !!info.targetElementId &&
                                            !options.devDisableDeepSearch,
                                        supportsDeepSearchModifier:
                                            !!info.modifiers,
                                        deepSearchBigger:
                                            !!info.modifiers?.includes('Shift'),
                                        deepSearchBiggerLimit:
                                            options.deepSearchBiggerLimit,
                                    ...
                                    .catch((e) => {
                                        console.error(
                                            'xIFr: Sending parseImage message failed! \n' +
                                                e
                                        );
                                    });
                            })
                            .catch((err) => {
                                console.error(
                                    `xIFr: storage.session or sendMessage(parseImage) error: ${err}`
                                );
                            });
                    })
                    .catch((err) => {
                ...
                console.error(
                    `xIFr: Can't run browser.scripting.executeScript. Missing "scripting" or "activeTab" permission?`
                );
            }
        } else {
            context.warn(
                'No image found in context menu click.' + JSON.stringify(info)
            );
        }
    }
});
...
browser.runtime.onMessage.addListener(function messageHandler(
    message,
    sender,
    sendResponse
) {
    if (['fetchdata', 'fetchdataBase64'].includes(message.message)) {
        // backend fetch
        ...
        function fetchdata_error(error) {
            console.error(
                'xIFr: Background ' + message.message + ' error!',
                error
            );
            if (error.name === 'TimeoutError' || error.name === 'AbortError') {
                console.error(
                    'xIFr: Abort - likely timeout - when reading image-data from ' +
                        url
                );
                result.error = browser.i18n.getMessage('fetchImageAbortError');
            } else {
                console.error(
                    'xIFr: fetch-ERROR trying to read image-data from ' +
                        url +
                        ' : ' +
                        error
                );
                result.error = browser.i18n.getMessage('fetchImageError');
                result.info = browser.i18n.getMessage(
                    'fetchFileWorkAroundInfo'
                );
            }
        ...
        if (message.message === 'fetchdata') {
            ...
            // re-fetching the image to get the binary data
            fetch(url.href, fetchOptions)
                .then(function (response) {
                    if (response.ok) {
                        ...
                        result.contentType =
                            response.headers.get('Content-Type') || ''; // https://developer.mozilla.org/en-US/docs/Web/Media/Formats/Image_types
                        result.lastModified =
                ...
                .then(function (arrayBuffer) {
                    if (arrayBuffer) {
                        context.debug(
                            'Looking at the fetch response (arrayBuffer)...'
                        );
                        context.info(
                            'headers.byteLength: ' + result.byteLength
                        );
                        context.info(
                            'arraybuffer.byteLength: ' + arrayBuffer.byteLength
                        );
                        context.info(
                            " *** Now doing a 'new Uint8Array(arrayBuffer)' in backend! ***"
                        );
                        result.byteArray = new Uint8Array(arrayBuffer);
                        result.byteLength =
                            arrayBuffer.byteLength || result.byteLength;
                    }
                    sendResponse(result);
                })
                .catch(fetchdata_error);
            return true; // Tell it to expect a later response (to be sent with sendResponse())
        } else if (message.message === 'fetchdataBase64') {
            // Probably a Chromium browser
            ...
            fetch(url.href, fetchOptions)
                ...
                .then(
                    ...
                    function (base64) {
                        if (base64) {
                            context.debug(
                                'Looking at the fetch response (base64)...'
                            );
                            context.info(
                                'headers.byteLength: ' + result.byteLength
                            );
                            result.byteLength =
                                result.byteLength ||
                                Math.round(base64.length / 1.36667); // Approximation
    ...
    } else if (message.message === 'EXIFready') {
        // 1st msg, create popup
        ...
        if (popupData.properties.URL?.startsWith('blob:http')) {
            popupData.warnings.push(
                browser.i18n.getMessage('displayBlobTrouble')
            );
        }
        if (
            popupData.properties.URL?.startsWith('file:') &&
            context.isFirefox()
        ) {
            popupData.warnings.push(
                browser.i18n.getMessage('displayFileTrouble')
            );
            // TODO: Is it actually possible to display a local image with URL.createObjectURL(blob) ?
        ...
        browser.storage.session.set({ popupData: popupData }).then(() => {
            browser.storage.session.get().then(({ previous, winpop }) => {
                if (
                    previous?.imgURL &&
                    previous.imgURL === message.properties.URL &&
                    !message.properties.URL.startsWith('file:')
                ) {
                    context.debug(
                        'Previous popup was same - Focus to previous if still open...'
                    );
                    browser.windows
                        .update(previous.winId, { focused: true })
                        .then(() => {
                            context.debug(
                                'Existing popup was attempted REfocused.'
                            );
                        })
                        .catch(() => {
                            context.debug(
                                "REfocusing didn't succeed. Creating a new popup..."
                            );
                            createPopup(message, winpop);
                        });
                } else {
                    if (previous?.winId) {
                        // But it would be smarter to just re-use an existing popup than to close previous and open a new?
                        browser.windows
                            .remove(previous.winId)
                            .then(() => {
                                context.debug(
                                    'Popup with id=' +
                                        previous.winId +
        ...
        // 2nd msg, populate popup

        browser.storage.session.get('popupData').then(function (data) {
...
function createPopup(request, popupPos) {
    // Called when 'EXIFready'
    ...
    if (!win?.width) {
        console.error(
            'xIFr: Current browser-window properties not received by backgroundscript'
        );
    }
    if (!scr?.availWidth) {
        console.error(
            'xIFr: window.screen properties not received by backgroundscript'
        );
    }
    switch (popupPos) {
        case 'center':
        ...
        case 'topRightBrowser':
            if (win?.width) {
                pos = {
                    left: win.left + win.width - 650 - 10,
                    top: win.top + 10,
                };
            }
            break;
        case 'leftish':
            if (win?.width) {
                pos = {
                    ...
                    top: Math.max(
                        win.top + Math.floor(win.height / 2) - 350,
                        10
                    ),
                };
            }
        ...
        case 'rightish':
            if (win?.width || scr?.availWidth) {
                pos = {
                    left: Math.min(
                        win.left + win.width - 450,
                        scr.availWidth - 650 - 10
                    ),
                    top: Math.max(
                        win.top + Math.floor(win.height / 2) - 350,
                        10
    ...
    browser.windows
        ...
        .then((popwin) => {
            browser.storage.session.set({
                previous: { winId: popwin.id, imgURL: request.properties.URL },
            });
        });
}
    ...
    browser.contextMenus.create(
        ...
        () => {
            if (browser.runtime.lastError) {
                ...
                context.log(
                    'Menu-item probably already created: ' +
