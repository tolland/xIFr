...
import dataCleaner from '../utils/cleaner.js';
...
function supportsRegexLookAheadLookBehind() {
    try {
        return (
            'hibyehihi'.replace(/(?<=hi)hi/gu, 'hello').replace(/hi(?!bye)/gu, 'hey') ===
            'hibyeheyhello'
        );
    } catch (error) {
        return false;
...
const PURLsource =
    '(?<=[\\[:;,({\\s]|^)((http|https):\\/\\/)?[a-z0-9][-a-z0-9.]{1,249}\\.[a-z][a-z0-9]{1,62}\\b([-a-z0-9@:%_+.~#?&/=]*)'; // PURL.source
const PURLflags = 'imu'; // PURL.flags
const PEMAILsource =
    '(?<=[\\[:;,({\\s]|^)(mailto:)?([a-z0-9._-]+@[a-z0-9][-a-z0-9.]{1,249}\\.[a-z][a-z0-9]{1,62})'; // PEMAIL.source
...
function linkifyWithNodeAppendables(str, anchorattributes, emailanchorattributes) {
    ...
    function mailtoAndHttpLinks(str) {
        ...
        if (e === null) {
            ...
            return [
                ...httpLinks(begin),
                createRichElement('a', emailattrib, demail),
                ...mailtoAndHttpLinks(end),
            ]; // (recursive)
        }
    }
...
if (!supportsRegexLookAheadLookBehind()) {
    linkifyWithNodeAppendables = function (str, ...ignore) {
        return str;
    };
    console.warn(
        "Current webbrowser doesn't support regexp-lookbehind (or lookahead)! DOMUtil.linkifyWithNodeAppendables(str) will return str parameter unchanged."
    );
}
...
function formatWithNodeAppendables(s, linesplitter, anchorattributes, emailanchorattributes) {
    ...
    for (let i = lines.length - 1; i > 0; i--) {
        ...
        lines.splice(
            i + 1,
            1,
            ...linkifyWithNodeAppendables(lines[i + 1], anchorattributes, emailanchorattributes)
        );
    }
    return [
        ...linkifyWithNodeAppendables(lines[0], anchorattributes, emailanchorattributes),
        ...lines.slice(1),
    ];
}
...
function cleanAndFormatWithNodeAppendables(s) {
    // Needs a better name? :-)
...
function flickrEmbeddedId(imageUrl) {
    return /^https?:\/\/(?:farm|live)\d*\.static\.?flickr\.com\/\d+\/(?<imageId>\d+)_[0-9a-f]{4,}(?:_[a-z0-9]{1,3})?\.[a-z]{3,4}(?:$|\?|#|\/)/iu.exec(
        imageUrl
    )?.groups.imageId;
...
function flickrOriginId(imageUrl) {
    ...
    return /\/(?<imageId>\d+)_[0-9a-f]{4,}_[a-z0-9]{1,3}\.[a-z]{3,4}(?:$|\?|#|\/)/iu.exec(path)
        ?.groups.imageId;
}

/*
1. Registers shortcut Esc to close popup
2. create a thumnail size method
 */
function populate(response) {
    console.log('running in populate');
    keyShortcuts.register('Escape', function closePopup() {
        self.close();
    });
    ...
    if (response.properties.URL) {
        ...
        if (response.properties.naturalWidth) {
            const ts = thumbsize(
                response.properties.naturalWidth,
                response.properties.naturalHeight
            );
        ...
        function linkProperties(imageUrl) {
            ...
            if (imageUrl.startsWith('data:')) {
                ...
                textContent =
                    (linkElem.pathname.length > 1
                        ? linkElem.pathname.substring(linkElem.pathname.lastIndexOf('/') + 1)
                        : linkElem.hostname || linkElem.host) || '[ ./ ]';
                title = imageUrl;
            }
            return {
                url: imageUrl,
        ...
        if (response.properties.pageShownURL) {
            ...
            if (response.properties.pageShownType) {
                document.getElementById('orig_type').textContent =
                    '(' + response.properties.pageShownType + ')';
            }
        ...
        document.getElementById('imgsize2').textContent =
            response.properties.byteLength >= 1048576
                ? (response.properties.byteLength / 1048576).toFixed(2).toString() + ' MB'
                : (response.properties.byteLength / 1024).toFixed(2).toString() + ' KB';
        ...
        if (flickrId && badge) {
            ...
            if (flickrId) {
                badge.href = `https://flickr.com/photo.gne?id=${flickrId}`;
                badge.firstElementChild.title =
                    'Filename hints it might originate from Flickr. Click to look for photopage...';
    ...
    function addMessages(list, icon, alt) {
        list.forEach(function (item) {
            ...
            document
                ...
                .appendChild(createRichElement('div', {}, sign, ' ', msg));
        });
    }
    ...
    function gpsRowClick(event) {
        ...
        document.querySelectorAll('.gps.expandable').forEach(function (elm) {
            const row = elm.parentNode.parentNode;
            row.removeAttribute('title');
            row.classList.remove('clickable', 'notice');
            row.removeEventListener('click', gpsRowClick, {
                capture: true,
                once: true,
            });
        });
    ...
    function listArrayWithNodeAppendables(arr) {
        // Inserting linebreaks to get one item pr. line
        const ret = [];
    ...
    function addDataRow(key_v) {
        if (
            key_v !== 'GPSPureDdLat' &&
            key_v !== 'GPSPureDdLon' &&
            key_v !== 'AdditionalSoftware' &&
            response.data[key_v].value !== null &&
            response.data[key_v].value !== ''
        ) {
            ...
            if (
                ...
                value.insertAdjacentElement(
                    ...
                    createRichElement(
                        'span',
                        { class: 'gps expandable' },
                        document.createElement('br'),
                        response.data['GPSPureDdLat'].value + ' (decimal)'
                    )
                );
                row.title = 'Click for decimal latitude and longitude values';
                row.classList.add('clickable', 'notice');
                row.addEventListener('click', gpsRowClick, {
                    capture: true,
                    once: true,
                });
                ...
                value.insertAdjacentElement(
                    ...
                    createRichElement(
                        ...
                        { class: 'gps expandable' },
                        document.createElement('br'),
                        response.data['GPSPureDdLon'].value + ' (decimal)'
                    )
                );
                row.title = 'Click for decimal latitude and longitude values';
                row.addEventListener('click', gpsRowClick, {
                ...
                value.insertAdjacentElement(
                    ...
                    createRichElement(
                        'span',
                        { class: 'software expandable' },
                        ...listArrayWithNodeAppendables(response.data['AdditionalSoftware'].value)
                    )
                );
                row.title = 'Click for additional software used';
                row.addEventListener('click', softwareRowClick, {
                    capture: true,
                    once: true,
                });
                row.classList.add('clickable', 'notice');
                ...
                row.title =
                    'Notice: Color space given in Exif and XMP meta-data, might not be the same as actual image color space used!';
                row.classList.add('notice');
            } else if (
                typeof response.data[key_v].value === 'string' &&
                response.data[key_v].value.startsWith('{') &&
                response.data[key_v].value.endsWith('}')
            ) {
                try {
                    // const obj = JSON.parse(response.data[key_v].value);
                    var tempVal = dataCleaner(key_v, response.data[key_v].value);
                    let formattedValue = JSON.stringify(JSON.parse(tempVal), null, 4);
                    // value.textContent = JSON.stringify(obj, null, 2);
                    //codeElem = document.createElement('pre');
                    var codeElem = document.createElement('code');
                    codeElem.innerHTML = syntaxHighlight(formattedValue);
                    value.textContent = null;
                    //codeElem.textContent = syntaxHighlight(formattedValue);
                    //codeElem.classList.add('code_block');
                    value.append(codeElem);
                } catch (e) {
                    console.error(e);
                }
            }
    ...
    function showMapTab(event) {
        ...
        document.getElementById('osmap').src =
            'https://www.openstreetmap.org/export/embed.html?bbox=' +
            (response.data.GPSPureDdLon.value - 0.003) +
            '%2C' +
            (response.data.GPSPureDdLat.value - 0.007) +
            '%2C' +
            (response.data.GPSPureDdLon.value + 0.003) +
            '%2C' +
            (response.data.GPSPureDdLat.value + 0.007) +
            '&layer=mapnik&marker=' +
            response.data.GPSPureDdLat.value +
            '%2C' +
            response.data.GPSPureDdLon.value;
        document.getElementById('largermap').href =
    ...
    function displayInPage(event) {
        if (document.querySelector('#image img.toggleInPage')) {
            browser.tabs
                .sendMessage(response.properties.tabId, {
    ...
    if (
        response.data.GPSPureDdLat &&
        response.data.GPSPureDdLon &&
        typeof response.data.GPSPureDdLat.value === 'number' &&
        typeof response.data.GPSPureDdLon.value === 'number'
    ) {
        ...
        if (maplinks) {
            ...
            maplinks.appendChild(
                maplink(
                    'Locate on OpenStreetMap',
                    'OSM',
                    `https://www.openstreetmap.org/?mlat=${lat}&mlon=${lon}&layers=M`,
                    'O'
                )
            );
            maplinks.appendChild(
            ...
            maplinks.appendChild(
                maplink(
                    'Locate on Bing Maps',
                    'Bing',
                    `https://www.bing.com/maps/?cp=%lat%~%lon%&lvl=16&sp=point.${lat}_${lon}_${titleString}`,
                    'B'
                )
            );
            maplinks.appendChild(
                maplink(
                    'Locate on MapQuest',
                    'MapQuest',
                    `https://www.mapquest.com/latlng/${lat},${lon}`,
                    'Q'
                )
            );
            maplinks.appendChild(
                maplink(
                    'Locate on Here WeGo',
                    'Here',
                    `https://share.here.com/l/${lat},${lon}`,
                    'H'
                )
            );
            maplinks.appendChild(
                maplink(
                    'Explore nearby on Flickr',
                    'Flickr',
                    `https://www.flickr.com/map/?fLat=${lat}&fLon=${lon}&zl=15`,
                    'F'
                )
            ); // "https://www.flickr.com/map/?fLat=${lat}&fLon=${lon}&zl=15&everyone_nearby=1"  -  &zl=15&min_upload_date=2019-06-07%2000%3A00%3A00&max_upload_date=2019-07-08%2000%3A00%3A00 ?
            maplinks.appendChild(
                maplink(
                    'Investigate more on GeoHack',
                    'GeoHack',
                    `https://geohack.toolforge.org/geohack.php?language=en&params=${lat};${lon}`,
                    'X'
                )
            );
        }
...
var downloadData = () => {
    var content = document.getElementById('data').innerText;
    //console.dir(content);
...
function setup(options) {
    ...
    document.getElementById('acDownload').addEventListener('click', downloadData, true);
}
